    ...
    public function handle(Request $request, Closure $next)
    {
        \Menu::make('HopeUIMenu', function ($menu) use($request) {
            ...
            $moduleMenu = $this->parentMenu($menu, [
                'icon' => '<svg width="20" class="icon-20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" fill-rule="evenodd" clip-rule="evenodd" d="M5.91064 20.5886C5.91064 19.7486 6.59064 19.0686 7.43064 19.0686C8.26064 19.0686 8.94064 19.7486 8.94064 20.5886C8.94064 21.4186 8.26064 22.0986 7.43064 22.0986C6.59064 22.0986 5.91064 21.4186 5.91064 20.5886ZM17.1606 20.5886C17.1606 19.7486 17.8406 19.0686 18.6806 19.0686C19.5106 19.0686 20.1906 19.7486 20.1906 20.5886C20.1906 21.4186 19.5106 22.0986 18.6806 22.0986C17.8406 22.0986 17.1606 21.4186 17.1606 20.5886Z" fill="currentColor"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M20.1907 6.34909C20.8007 6.34909 21.2007 6.55909 21.6007 7.01909C22.0007 7.47909 22.0707 8.13909 21.9807 8.73809L21.0307 15.2981C20.8507 16.5591 19.7707 17.4881 18.5007 17.4881H7.59074C6.26074 17.4881 5.16074 16.4681 5.05074 15.1491L4.13074 4.24809L2.62074 3.98809C2.22074 3.91809 1.94074 3.52809 2.01074 3.12809C2.08074 2.71809 2.47074 2.44809 2.88074 2.50809L5.26574 2.86809C5.60574 2.92909 5.85574 3.20809 5.88574 3.54809L6.07574 5.78809C6.10574 6.10909 6.36574 6.34909 6.68574 6.34909H20.1907ZM14.1307 11.5481H16.9007C17.3207 11.5481 17.6507 11.2081 17.6507 10.7981C17.6507 10.3781 17.3207 10.0481 16.9007 10.0481H14.1307C13.7107 10.0481 13.3807 10.3781 13.3807 10.7981C13.3807 11.2081 13.7107 11.5481 14.1307 11.5481Z" fill="currentColor"></path></svg>',
                'title' => 'E-Commerce',
                'nickname' => 'ecommerce',
                'order' => 270
            ]);
            ...
            /* Start If Condition */
            if(str_contains($request->path(), 'ecommerce')) {
                // Static Social
                $this->staticMenu($moduleMenu, ['title' => 'E-COMMERCE', 'order' => 405]);

                //Static Pages
                $this->staticMenu($moduleMenu, ['title' => 'PAGES', 'order' => 415]);

                // Static Sidebar End
                $this->staticMenu($moduleMenu, ['title' => 'OTHER', 'order' => 430]);
            ...
            // Static E-COMMERCE
            $this->mainRoute($moduleMenu, [
                'icon' => '<svg width="20" class="icon-20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.14373 20.7821V17.7152C9.14372 16.9381 9.77567 16.3067 10.5584 16.3018H13.4326C14.2189 16.3018 14.8563 16.9346 14.8563 17.7152V20.7732C14.8562 21.4473 15.404 21.9951 16.0829 22H18.0438C18.9596 22.0023 19.8388 21.6428 20.4872 21.0007C21.1356 20.3586 21.5 19.4868 21.5 18.5775V9.86585C21.5 9.13139 21.1721 8.43471 20.6046 7.9635L13.943 2.67427C12.7785 1.74912 11.1154 1.77901 9.98539 2.74538L3.46701 7.9635C2.87274 8.42082 2.51755 9.11956 2.5 9.86585V18.5686C2.5 20.4637 4.04738 22 5.95617 22H7.87229C8.19917 22.0023 8.51349 21.8751 8.74547 21.6464C8.97746 21.4178 9.10793 21.1067 9.10792 20.7821H9.14373Z" fill="currentColor"></path></svg>',
            ...
            $this->childMain($shop, [
                'shortTitle' => 'SM',
                'title' => 'Shop Main',
                'route' => 'e-commerce.shop-main',
                'active' => 'ecommerce/shop-main',
            ]);
            $this->childMain($shop, [
                'shortTitle' => 'SL',
                'title' => 'Shop - Left Filter',
                'route' => 'e-commerce.shop-left-filter',
                'active' => 'ecommerce/shop-left-filter',
            ]);
            $this->childMain($shop, [
                'shortTitle' => 'SR',
                'title' => 'Shop - Right Filter',
                'route' => 'e-commerce.shop-right-filter',
                'active' => 'ecommerce/shop-right-filter',
            ]);
            $this->childMain($shop, [
                'shortTitle' => 'PG',
                'title' => 'Product Grid View',
                'route' => ['e-commerce.products', 'type' => 'product-grid']
            ]);
            $this->childMain($shop, [
                'shortTitle' => 'PL',
                'title' => 'Product List View',
                'route' => ['e-commerce.products', 'type' => 'products-list']
            ]);
            $this->childMain($shop, [
                'shortTitle' => 'C',
                'title' => 'Categories List',
                'route' => 'e-commerce.categories-list',
                'active' => 'ecommerce/categories-list',
            ]);

            // Product Details
            $product = $this->parentMenu($moduleMenu, [
                'icon' => '<svg width="20" class="icon-20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" d="M16.34 1.99976H7.67C4.28 1.99976 2 4.37976 2 7.91976V16.0898C2 19.6198 4.28 21.9998 7.67 21.9998H16.34C19.73 21.9998 22 19.6198 22 16.0898V7.91976C22 4.37976 19.73 1.99976 16.34 1.99976Z" fill="currentColor"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M11.1246 8.18921C11.1246 8.67121 11.5156 9.06421 11.9946 9.06421C12.4876 9.06421 12.8796 8.67121 12.8796 8.18921C12.8796 7.70721 12.4876 7.31421 12.0046 7.31421C11.5196 7.31421 11.1246 7.70721 11.1246 8.18921ZM12.8696 11.362C12.8696 10.88 12.4766 10.487 11.9946 10.487C11.5126 10.487 11.1196 10.88 11.1196 11.362V15.782C11.1196 16.264 11.5126 16.657 11.9946 16.657C12.4766 16.657 12.8696 16.264 12.8696 15.782V11.362Z" fill="currentColor"></path></svg>',
            ...
            // Submenu- Product - Child
            $this->childMain($product,[
                'shortTitle' => 'P',
                'title' => 'Product Detail',
                'route' =>  'e-commerce.product-detail',
                'active' =>  'ecommerce/product-detail',
            ]);
            $this->childMain($product,[
                'shortTitle' => '3D',
                'title' => '3D Product Detail',
                'route' =>  'e-commerce.product-detail-3d',
                'active' =>  'ecommerce/product-detail-3d',
            ]);
            $this->childMain($product,[
                'shortTitle' => 'P',
                'title' => '360 Product Detail',
                'route' =>  'e-commerce.product-detail-360',
                'active' =>  'ecommerce/product-detail-360',
            ]);
