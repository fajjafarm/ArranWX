...
(function () {
  ...
  // class add function
  IQUtils.addClass = function (elem) {
  ...
  IQUtils.removeClass = function (elem) {
  ...
  // create event
  IQUtils.createEvent = function (eventName, eventData) {
    return new Event(eventName, eventData);
  };

  // deep merge function
  IQUtils.mergeDeep = function (target) {
    ...
    return IQUtils.mergeDeep.apply(IQUtils, [target].concat(sources));
  };

  // get style root variables function
  IQUtils.getRootVars = function (property) {
  ...
  IQUtils.setRootVariables = function (variables) {
  ...
  IQUtils.removeRootVariables = function (variables) {
    var _root = document.documentElement;
    var _variables = variables;
    _.forEach(_variables, function (value, key) {
  ...
  // color mix function
  IQUtils.colorMix = function (color_1, color_2, weight) {
    function d2h(d) {
  ...
  // tint color function
  IQUtils.tintColor = function (color, weight) {
    weight = typeof weight !== 'undefined' ? weight : 50; // set the weight to 50%, if that argument is omitted
    var color = color;
  ...
  IQUtils.shadeColor = function (color, weight) {
    weight = typeof weight !== 'undefined' ? weight : 50; // set the weight to 50%, if that argument is omitted
    var color = color;
    var color_1 = color.substr(1); // strip the '#' character from the beginning
    var color_2 = '000000';
  ...
  IQUtils.getColorShadeTint = function (color, value, dark) {
    var colors = {};
    if (dark) {
  ...
  IQUtils.setFontFamily = function (fontFamily, type) {
    // add font into head
  ...
  IQUtils.debounce = function (func, wait, immediate) {
    var timeout;
    return function () {
      var context = this,
        args = arguments;
