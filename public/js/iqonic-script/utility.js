...
(function () {
     function IQUtils (params) {
    }


    // save session storage value
    IQUtils.saveSessionStorage = function (key, value) {
        window.sessionStorage.setItem(key, value);
    }

    // get session storage value
    IQUtils.getSessionStorage = function (key) {
        return window.sessionStorage.getItem(key);
    }

    // remove session storage value
    IQUtils.removeSessionStorage = function (key) {
        window.sessionStorage.removeItem(key);
    }

    // save local storage value
    IQUtils.saveLocalStorage = function (key, value) {
        window.localStorage.setItem(key, value);
    }

    // get local storage value
    IQUtils.getLocalStorage = function (key) {
        return window.localStorage.getItem(key);
    }

    // remove local storage value
    IQUtils.removeLocalStorage = function (key) {
        window.localStorage.removeItem(key);
    }

    // get cookie value
    IQUtils.getCookie = function (cname) {
        var name = cname + "=";
    ...
    // set cookie value
    IQUtils.setCookie = function (cname, cvalue, exdays) {
        var d = new Date();
    ...
    // remove cookie value
    IQUtils.removeCookie = function (cname) {
        document.cookie = cname + '=; expires=Thu, 01 Jan 1970 00:00:01 GMT;';
    }

    // check storage key exist function
    IQUtils.checkStorageKey = function (key) {
    ...
    IQUtils.checkSessionStorageKey = function (key) {
    ...
    IQUtils.checkCookieKey = function (key) {
    ...
    IQUtils.checkValue = function (value) {
    ...
    IQUtils.checkAllStorageKey = function (key) {
    ...
    IQUtils.checkStorageArray = function (key, storages) {
        let result = false;
        let obj = {}
    ...
    IQUtils.getUrlParameter = function (name) {
    ...
    IQUtils.getQueryString = function (name) {
        name = name.replace(/[\[]/, '\\[').replace(/[\]]/, '\\]');
        const regex = new RegExp('[\\?&]' + name + '=([^&#]*)');
        const results = regex.exec(location.search);
        return results === null ? '' : decodeURIComponent(results[1].replace(/\+/g, ' '));
    ...
    IQUtils.getElem = function (selector, elem = document) {
        return elem.querySelector(selector);
    }


    IQUtils.getElems = function (selector, elem = document) {
        return elem.querySelectorAll(selector);
    }

    IQUtils.setContent = function (selector, content) {
    ...
    // class add function
    IQUtils.addClass = function (elem,...className) {
        let _newElem = elem
        if(_.isString(_newElem)){
    ...
    IQUtils.removeClass = function (elem,...className) {
        let _newElem = elem
        if(_.isString(_newElem)){
            _newElem = IQUtils.getElems(elem);;
        }
    ...
    IQUtils.toggleClass = function (elem,className) {
        elem.classList.toggle(className);
    }

    // class has function
    IQUtils.hasClass = function (elem,className) {
        return elem.classList.contains(className);
    }

    // get attribute value
    IQUtils.getAttr = function (elem,attr) {
        return elem.getAttribute(attr);
    }

    // set attribute value
    IQUtils.setAttr = function (elems,object) {
        let _newElem = elems
        if(_.isString(_newElem)){
    ...
    IQUtils.removeAttr = function (elem,attr) {
        elem.removeAttribute(attr);
    }

    // update style value
    IQUtils.setStyle = function (elems,object) {
    ...
    IQUtils.getPosition = function (elem) {
        var xPosition = 0;
        var yPosition = 0;
    ...
    // get element width
    IQUtils.getWidth = function (elem) {
        return elem.offsetWidth;
    }

    // get element height
    IQUtils.getHeight = function (elem) {
        return elem.offsetHeight;
    }

    // create event
    IQUtils.createEvent = (eventName, eventData) => {
        return new Event(eventName, eventData);
    }

    // deep merge function
    IQUtils.mergeDeep = (target, ...sources) => {
    ...
    IQUtils.getRootVars = (property, elem = document.body) => {
        let _newElem = elem
            if(_.isString(_newElem)){
                _newElem = IQUtils.getElems(elems);;
            }
    ...
    IQUtils.setRootVariables = (variables) => {
    ...
    IQUtils.removeRootVariables = (variables) => {
        const _root = document.documentElement;
        const _variables = variables;
        _.forEach(_variables, function (value, key) {
    ...
    // color mix function
    IQUtils.colorMix = (color_1, color_2, weight) => {
    ...
    // tint color function
    IQUtils.tintColor = (color, weight) => {
        weight = (typeof(weight) !== 'undefined') ? weight : 50; // set the weight to 50%, if that argument is omitted
        var color = color;
    ...
    IQUtils.shadeColor = (color, weight) => {
        weight = (typeof(weight) !== 'undefined') ? weight : 50; // set the weight to 50%, if that argument is omitted
        var color = color;
        var color_1 = color.substr(1); // strip the '#' character from the beginning
        var color_2 = '000000';
    ...
    IQUtils.getColorShadeTint = (color, value, dark) => {
        let colors = {}
        if(dark) {
    ...
    IQUtils.setFontFamily = (fontFamily, type) => {
        // add font into head
    ...
    IQUtils.debounce = (func, wait, immediate) => {
        let timeout
        return function() {
